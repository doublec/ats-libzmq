...
implement main () = {
  fun loop_requests {l:agz} {n,m:nat | m >= n} .< m-n >. (requester: !zmqsocket l, n: int n, max: int m): void =
    if n = max then
      ...
      val r = zmq_msg_init_size (pf_request | &request, string1_length(s))
      val () = assertloc (r = 0)

      val (pf_data, fpf_data | p_data) = zmq_msg_data (pf_request | &request)
      ...
      prval () = fpf_data(pf_data, pf_request)

      val () = printf("Sending Hello %d...\n", @(n))
      val r = zmq_send (pf_request | requester, &request, 0)
      val () = assertloc (r = 0)
      ...
      val r = zmq_msg_init (pf_reply | &reply)
      val () = assertloc (r = 0)

      val r = zmq_recv (pf_reply | requester, &reply, 0)
      val () = assertloc (r = 0)
